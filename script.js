document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. STATE MANAGEMENT ---
    let cvData = {
        layout: 'portfolio', // <- Puesto por defecto para ver el cambio
        themeColor: '#dc3545',
    ...
    (function buildTemplateAndFormFunctions(){
        ...
        templates.creative = (data) => `<div id="cv-template" style="height:100%; display:flex; position:relative;"><div style="position:absolute; top:0; left:0; right:0; height: 200px; background:${data.themeColor};"></div><div style="width:30%; padding:2rem; color:white; z-index:1; display:flex; flex-direction:column; justify-content:flex-end; padding-bottom: 120px;"><h1 style="font-family:var(--font-heading); font-size:2.5rem; line-height:1.1; text-shadow: 1px 1px 3px rgba(0,0,0,0.3);">${data.personalInfo.firstName || ''}<br>${data.personalInfo.lastName || ''}</h1><h2 style="font-family:var(--font-heading); font-size:1rem; font-weight:500; margin-top:0.5rem;">${data.personalInfo.title||''}</h2></div><div style="width:70%; padding:2rem; background:white; z-index:1; margin-top: 100px;"><div style="width:180px; height:180px; border-radius:50%; overflow:hidden; border: 8px solid white; box-shadow: 0 0 20px rgba(0,0,0,0.2); margin-top: -110px; margin-left:-70px; background-color:${data.themeColor};">${renderAvatar(data)}</div><main style="margin-top:2rem;">${renderGenericSection('Resumen', data.personalInfo.summary ? [{text: data.personalInfo.summary}] : [], item => `<p style="font-size:.8rem;line-height:1.6;white-space:pre-wrap">${item.text}</p>`, data.themeColor)}${renderGenericSection('Experiencia', data.experience, renderExperienceItem, data.themeColor)}${renderGenericSection('Educación', data.education, renderEducationItem, data.themeColor)}${renderGenericSection('Habilidades', data.skills, s => `<p>${s.name} - ${levelLabels[s.level]}</p>`, data.themeColor)}</main></div></div>`;
        ...
        templates.midnight = (data) => `<div id="cv-template" style="height:100%; background:#1a1a1a; color:#e0e0e0; padding:2.5rem; display:flex; flex-direction:column;"><header style="display:flex; align-items:center; gap:1.5rem; padding-bottom:1.5rem; border-bottom:1px solid #444;"><div style="width:100px; height:100px; border-radius:50%; overflow:hidden; border:3px solid ${data.themeColor};">${renderAvatar(data)}</div><div><h1 style="font-family:var(--font-heading); font-size:2.5rem; color:white;">${getFullName(data.personalInfo)}</h1><h2 style="font-size:1.1rem; color:${data.themeColor};">${data.personalInfo.title}</h2></div></header><main style="flex-grow:1; margin-top:1.5rem;">${renderGenericSection('Resumen', data.personalInfo.summary ? [{text: data.personalInfo.summary}] : [], item => `<p style="font-size:.9rem;line-height:1.6;white-space:pre-wrap">${item.text}</p>`, data.themeColor)}${renderGenericSection('Experiencia', data.experience, renderExperienceItem, data.themeColor)}${renderGenericSection('Educación', data.education, renderEducationItem, data.themeColor)}</main>${renderFooter(data, {color: '#ccc', bgColor: '#222', borderColor:'#444'})}</div>`;
        ...
        templates.prestige = (data) => `<div id="cv-template" style="height:100%; padding:3rem; display:flex; background:#f9f9f9;"><aside style="width:35%; padding-right:2rem; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;"><div style="width:120px; height:120px; border:2px solid ${data.themeColor}; color:${data.themeColor}; font-size:4rem; font-weight:600; display:flex; align-items:center; justify-content:center; margin-bottom:1rem;">${getInitials(data.personalInfo)}</div><h1 style="font-family:var(--font-serif); font-size:2rem; line-height:1.2;">${getFullName(data.personalInfo)}</h1><h2 style="font-size:1rem; font-weight:400;">${data.personalInfo.title}</h2></aside><main style="width:65%; padding-left:2rem; border-left:1px solid #ddd;">${renderGenericSection('Perfil', data.personalInfo.summary ? [{text: data.personalInfo.summary}] : [], item => `<p style="font-size:.9rem; line-height:1.6;white-space:pre-wrap">${item.text}</p>`, data.themeColor)}${renderGenericSection('Experiencia', data.experience, renderExperienceItem, data.themeColor)}${renderGenericSection('Educación', data.education, renderEducationItem, data.themeColor)}</main></div>`;
        ...
        <div class="layout-card ${cvData.layout==='academic'?'active':''}" data-layout="academic"><img src="img/academico.png" alt="Académico"><p>Académico</p></div>
        <div class="layout-card ${cvData.layout==='visionary'?'active':''}" data-layout="visionary"><img src="img/visionario.png" alt="Visionario"><p>Visionario</p></div>
        <div class="layout-card ${cvData.layout==='swiss'?'active':''}" data-layout="swiss"><img src="img/suizo.png" alt="Suizo"><p>Suizo</p></div>
        <div class="layout-card ${cvData.layout==='midnight'?'active':''}" data-layout="midnight"><img src="img/medianoche.png" alt="Medianoche"><p>Medianoche</p></div>
        <div class="layout-card ${cvData.layout==='artisan'?'active':''}" data-layout="artisan"><img src="img/artesano.png" alt="Artesano"><p>Artesano</p></div>
        <div class="layout-card ${cvData.layout==='chronological'?'active':''}" data-layout="chronological"><img src="img/cronologico.png" alt="Cronológico"><p>Cronológico</p></div>
        <div class="layout-card ${cvData.layout==='impact'?'active':''}" data-layout="impact"><img src="img/impacto.png" alt="Impacto"><p>Impacto</p></div>
        <div class="layout-card ${cvData.layout==='portfolio'?'active':''}" data-layout="portfolio"><img src="img/portafolio.png" alt="Portafolio"><p>Portafolio</p></div>
        <div class="layout-card ${cvData.layout==='columnist'?'active':''}" data-layout="columnist"><img src="img/columnista.png" alt="Columnista"><p>Columnista</p></div>
        <div class="layout-card ${cvData.layout==='prestige'?'active':''}" data-layout="prestige"><img src="img/prestigio.png" alt="Prestigio"><p>Prestigio</p></div>
        <div class="layout-card ${cvData.layout==='monochrome'?'active':''}" data-layout="monochrome"><img src="img/monocromo.png" alt="Monocromo"><p>Monocromo</p></div>
        </div></div>`);};
        ...
        // === NUEVO FORM RENDERER PARA PORTAFOLIO ===
        formRenderers.portfolio = () => {
            renderForm(`<div class="form-section" data-section="portfolio">
                <h2 class="section-title">Portafolio</h2>
                <p class="section-subtitle">Muestra tus mejores trabajos visuales.</p>
                <div class="add-item-container">
                    <button class="btn btn-secondary" data-action="add" data-section="portfolio">+ Añadir Proyecto</button>
                </div>
                <div class="dynamic-list">
                    ${cvData.portfolio.map(item => `
                        <div class="item" data-id="${item.id}">
                            <div class="item-header">
                                <h4>${item.title || 'Nuevo Proyecto'}</h4>
                                <button class="btn-delete" data-action="delete" data-section="portfolio" data-id="${item.id}"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
                            </div>
                            <div style="display: flex; gap: 1rem; align-items: flex-start;">
                                <img src="${item.img || 'https://via.placeholder.com/100x75/e9ecef/6c757d?text=Vista'}" style="width: 100px; height: 75px; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--color-border);" class="portfolio-preview">
                                <div style="flex-grow: 1;">
                                    ...
                                    <div class="form-group" style="margin-bottom:0;">
                                        ...
                                        <input type="text" name="img" value="${item.img || ''}" placeholder="https://ejemplo.com/imagen.png">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    function init() {
        ...
        formWrapper.addEventListener('input', (e) => {
            ...
            if (section === 'personal') {
            ...
            } else if (['experience', 'education', 'impacts'].includes(section)) {
                ...
                const itemEl = target.closest('.item');
                const item = cvData.portfolio.find(i => i.id == itemEl.dataset.id);
                if(item) {
        ...
        formWrapper.addEventListener('click', (e) => {
            ...
            if (action) {
                 if (section === 'footer' && action === 'add') {
                    ...
                    if (cvData.avatar.type !== newType) { cvData.avatar = { type: newType, value: '' }; }
